...
def load_model(args, contexts, data_train):
    ...
    if mode == 'train':
        ...
        model_loaded = STTModule.load(prefix=model_path, epoch=model_num_epoch, context=contexts,
...
if __name__ == '__main__':
    ...
    if mode == 'train':
        data_names = [x[0] for x in data_train.provide_data]
        label_names = [x[0] for x in data_train.provide_label]
        module = STTModule(model_loaded, context=contexts, data_names=data_names, label_names=label_names)
