...
runme /usr/bin/time -f "%e" env CC=gcc-5 CXX=g++-5 make USE_OPENCV=1 USE_BLAS=openblas -j $(nproc) 2>&1 | tee build/cpu_compile_log.txt
echo "##############################"
echo "Starting make on GPU with g++5"
runme /usr/bin/time -f "%e" env CC=gcc-5 CXX=g++-5 make USE_OPENCV=1 USE_BLAS=openblas USE_CUDA=1 USE_CUDA_PATH=/usr/local/cuda USE_CUDNN=1 -j $(nproc) 2>&1 | tee build/gpu_compile_log.txt
